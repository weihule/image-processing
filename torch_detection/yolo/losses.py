...
class YoloV4Loss(nn.Module):
    ...
    def forward(self, preds, annotations):
        ...
        # all_preds shape:[batch_size,anchor_nums,85], 这里 [1:5] 回归部分已经变成预测框的左上和右下坐标(相对feature_map)
        # reg_preds format:[scaled_xmin,scaled_ymin,scaled_xmax,scaled_ymax]
        ...
        conf_loss, reg_loss, cls_loss = self.compute_batch_loss(all_preds, all_targets)
    ...
    def get_batch_anchors_targets(self, obj_reg_cls_heads, batch_anchors, annotations):
        """
        Assign a ground truth target for each anchor
        :param obj_reg_cls_heads: [[B, h, w, 3, 85], ...]  列表中三个元素
        :param batch_anchors: [[B,52,52,3,5], [B,26,26,3,5], ...]  列表中三个元素
        ...
        # [0,0,0,h1*w1*3,...,h2*w2*3,...]
        per_layer_prefix_ids = [0, 0, 0]
        # 分别遍历一个batch中所有图片的三个层级feature map
        # 经过这个for循环, 处理好了all_preds, all_anchors, 并初始化了all_targets
        for layer_idx, (per_level_heads, per_level_anchors) in enumerate(zip(obj_reg_cls_heads, batch_anchors)):
            ...
            # per level anchor shape: [B,H,W,3,5] -> [B,H*W*3,5]
            # 5: [grids_x_idx, grids_y_idx, relative_anchor_w, relative_anchor_h, stride]
            per_level_anchors = per_level_anchors.view(
                per_level_anchors.shape[0], -1, per_level_anchors.shape[-1])

            # per_level_heads: [B,H,W,3,(1+4+num_classes)] -> [B,H*W*3,(1+4+num_classes)]
            per_level_heads = per_level_heads.view(
                per_level_heads.shape[0], -1, per_level_heads.shape[-1])
            per_level_obj_preds = per_level_heads[..., 0:1]
            per_level_cls_preds = per_level_heads[..., 5:]

            # 根据per_level_heads解码出pred_bbox
            ...
            # per_level_scaled_xy_ctr = (2*per_level_heads[..., 1:3]-0.5) + per_level_anchors[..., 0:2]  # [B, H*W*3, 2]
        ...
        for img_idx, per_img_annots in enumerate(annotations):
            ...
            if one_image_annots.shape[0] > 0:
                # gt_classes index range from 0 to 79
                ...
                # anchor总数是 (h1*w1*3 + ...)
                # global_ids shape [gt_num, 9]
                # global_ids是根据gt_9_boxes_grid_xy找出在 all_anchor_num 中的idx
                # 每一个gt中心点在一个feature_map上会有一个对应的grid_cell, 而一个grid_cell有三个anchor,
                # 所以一个gt在三个feature_map上就有九个对应的anchor索引值, 故是 [gt_num, 9]
                ...
                # [gt_num, 9, 2]
                gt_9_boxes_scaled_wh = (gt_boxes[:, 2:4] - gt_boxes[:, 0:2]
                                        ).unsqueeze(1) / all_strides.unsqueeze(0).unsqueeze(-1)
                ...
                # positive_ious shape is [gt_num, 9]
                positive_ious = self.iou_function(gt_zero_ctr_9_boxes,
                                                  anchor_zero_ctr_9_boxes,
                                                  iou_type='IoU',
                                                  box_type='xyxy')

                # positive_anchor_idxs shape is [gt_num, ]
                _, positive_anchor_idxs = positive_ious.max(dim=1)

                # positive_anchor_idxs_mask shape is [gt_num, 9]
                # 通过bool()转换成布尔类型
                positive_anchor_idxs_mask = F.one_hot(
                    positive_anchor_idxs,
                    num_classes=anchor_sizes.shape[0]).bool()

                # positive_global_ids shape is [gt_num, ]
                positive_global_ids = global_ids[positive_anchor_idxs_mask].long()

                # gt_9_boxes_scale shape is [gt_num, 9, 2]
                gt_9_boxes_scale = gt_9_boxes_scaled_wh / feature_hw.unsqueeze(0)

                # positive_gt_9_boxes_scale shape is [gt_num, 2]
                # TODO: 这块需要理解一下mask是怎么操作的
                positive_gt_9_boxes_scale = gt_9_boxes_scale[positive_anchor_idxs_mask]

                # gt_9_scaled_boxes shape is [gt_num, 9, 4]
                gt_9_scaled_boxes = gt_boxes.unsqueeze(1) / all_strides.unsqueeze(0).unsqueeze(-1)

                # positive_gt_9_scaled_boxes shape is [gt_num, 4]
                ...
                # filter_ious shape is [gt_num, anchor_num]
                filter_ious = self.iou_function(pred_scaled_bboxes,
                                                gt_scaled_boxes,
                                                iou_type='IoU',
                                                box_type='xyxy')

                # 找出每一列的最大值, 即一个anchor与所有gt的iou的最大值
                filter_ious_max, _ = filter_ious.max(axis=0)
...
if __name__ == "__main__":
    anchor_sizes = [[10, 13], [16, 30], [33, 23], [30, 61],
                    ...
                    [373, 326]]
    strides = [8, 16, 32]
    anchor_sizes = torch.tensor(anchor_sizes).float()
    anchor_sizes = anchor_sizes.view(len(anchor_sizes) // 3, -1, 2)  # [3, 3, 2]
    # scale anchor size
    for i in range(anchor_sizes.shape[0]):
        anchor_sizes[i, :, :] = anchor_sizes[i, :, :] / strides[i]
    anchor_sizes = anchor_sizes.view(-1, 2)  # [9, 2]

    positive_anchor_idxs_mask = F.one_hot(torch.tensor([5, 1, 8]), num_classes=9).bool()
    print(positive_anchor_idxs_mask)
    global_ids = torch.randn((3, 9))
    print(global_ids)
    positive_global_ids = global_ids[positive_anchor_idxs_mask].long()
    print(positive_global_ids)
