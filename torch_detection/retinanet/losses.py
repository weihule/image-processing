...
def get_batch_anchors_annotations(batch_anchors, annotations):
    ...
    :returns  batch anchors annotations shape:[batch_size, anchor_nums, 5]
    """
...
class RetinaLoss(nn.Module):
    ...
    def forward(self, cls_heads, reg_heads, batch_anchors, annotations):
        ...
        :param annotations: (B, N, 5) 5:[x_min, y_min, x_max, y_max, label]
        :return:
    ...
    def compute_one_image_ciou_loss(self, per_image_reg_heads,
                                    ...
                                    per_img_anchors):
        """
        per_image_reg_heads: [anchor_num, 4] tx*, ty*, tw*, th*
        per_image_anchors_annotations: [anchor_num, 5], tx, ty, tw, th, label, 这里的label中有-1(无效样本)
                                       0(负样本) 1到80(正样本)
        per_img_anchors: [anchor_num, 4] x_min, y_min, x_max, y_max
        ...
        # 计算预测框和真实框的iou, 这里计算每一个预测框和对应gt的iou,
        # 所以最后计算的iou的数量和预测框(或真实框)的数量一致
        overlap_top_left = torch.max(annotations_bbox[:, :2], pred_bbox[:, :2])
        overlap_bot_right = torch.min(annotations_bbox[:, 2:], pred_bbox[:, 2:])
        overlap_area_sizes = torch.clamp(overlap_bot_right - overlap_top_left, min=0)
