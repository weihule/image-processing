    ...
    with torch.no_grad():
        ...
        for img_spilt_list in img_spilt_lists:
            ...
            for scores, classes, pred_bboxes, img, img_name, scale in \
                ...
                for class_id, bbox in zip(classes, pred_bboxes):
                    bbox = bbox / scale
                    pt1 = np.asarray([bbox[0], bbox[1]], dtype=np.int32)
                    pt2 = np.asarray([bbox[2], bbox[3]], dtype=np.int32)
                    print(int(class_id))
                    cv2.putText(img, voc_id2name[int(class_id)], pt1, cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 2)
