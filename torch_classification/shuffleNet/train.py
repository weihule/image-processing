...
def main(args):
    ...
    # train_set = MyDataSet(train_images_path, train_images_label, data_transform['train'])
    val_set = MyDataSet(val_images_path, val_images_label, data_transform['val'])
    ...
    optimizer = torch.optim.SGD(pg, lr=args.lr, momentum=0.9, weight_decay=4E-5)
    ...
    for epoch in range(args.epochs):
        ...
        for step, data in enumerate(train_loader):
            ...
            optimizer.step()
        print('mean_loss {}'.format(mean_loss.item() / (len(train_loader) + 1)))
        scheduler.step()
        ...
        #
        # save_path = os.path.join(save_root, 'model-{}.pth'.format(str(acc).strip('.')[:5]))
